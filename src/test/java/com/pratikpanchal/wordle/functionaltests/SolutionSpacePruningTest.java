        ...
        for (String word : words) {
            ...
            while (!hint.equals("GGGGG")) {
                ...
                String predictedWord =
                        advisor.suggestASolution(solutions, trial).orElse(null);

                /**
                 * suggestASolutionFromAllEquallyLikelySolutions randomly picks a wso that
                 * matches in
                 ...
                 *
                 */
//                String predictedWord = advisor.suggestASolutionFromAllEquallyLikelySolutions
//                (solutions, Optional.of(trial));

                /**
                 * Solution Space Preemptive Pruning
                 * If there are some number of positional locks (>0)
                 * and, if this is an attempt #5 (or less) and the current solution space has
                 * more than 2 solution candidates,
                 * it is time to invoke the SolutionSpacePreemptivePruner (a.k.a.
                 * ViableComputeEngine)
                 */
                if (ci.getNumberOfPositionalLocks() >= 2 && trial <= 5 && solutions.size() > 2) {

                    //List<Character> priorityChars = ci.getListOfPriorityCharactersFromStrings
                    // (solutions);
                    List<Character> priorityChars =
                            ci.getStrictListOfPriorityCharactersFromStrings(solutions);
                     ...
                     * is more than 1 (otherwise, there is no point in pruning as it won't prune
                     * anything
                ...
                if (trial == 5 || trial == 6) {
                    vsoTrace += "\nTrial: " + trial + "\n" + advisor.getAllWordScoreObjects(solutions, trial).toString();
                }
            ...
            solutionNumberOfTrialsToFreq.put(trial,
                    solutionNumberOfTrialsToFreq.getOrDefault(trial, 0) + 1);
        }

        log.info("Trial distribution: " + solutionNumberOfTrialsToFreq);
        ...
        assertEquals(12954, solved,
                "Solved " + solved + " words in 6 or less trials out of " + total +
                        " words");
    }
     ...
     * trial distribution:
     * {1=1, 2=161, 3=1837, 4=4444, 5=3692, 6=1843, 7=571, 8=264, 9=138, 10=79, 11=39, 12=23,
     * 13=10, 14=4, 15=1}
     ...
     * trial distribution:
     * {1=1, 2=169, 3=1930, 4=4346, 5=3634, 6=2076, 7=504, 8=235, 9=127, 10=49, 11=22, 12=7,
     * 13=3, 14=2, 15=1, 16=1}
     * Solved=12156 | Total=13107 | Success rate=92.74433508812085
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()>=3 && ci.getNumberOfPositionalLocks()<=4 && trial<=5 &&
     * solutions.size()>2)
     * trial distribution:
     ...
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()>=2 && ci.getNumberOfPositionalLocks()<=4 && trial<=5 &&
     * solutions.size()>2)
     * trial distribution
     ...
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()>=2 && ci.getNumberOfPositionalLocks()<=4 && trial<=4 &&
     * solutions.size()>2){
     * trial distribution
     * {1=1, 2=120, 3=1581, 4=4569, 5=4633, 6=1294, 7=504, 8=208, 9=101, 10=55, 11=29, 12=8,
     * 13=2, 14=1, 15=1}
     * Solved=12198 | Total=13107 | Success rate=93.06477454795147
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()>=1 && ci.getNumberOfPositionalLocks()<=4 && trial<=5 &&
     * solutions.size()>2)
     * trial distribution
     ...
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()%2==1 && ci.getNumberOfPositionalLocks()<=5 && trial<=5
     * && solutions.size()>2)
     * trial distribution
     * {1=1, 2=98, 3=1557, 4=4427, 5=3990, 6=1970, 7=599, 8=240, 9=110, 10=57, 11=30, 12=15,
     * 13=7, 14=5, 15=1}
     * Solved=12043 | Total=13107 | Success rate=91.8822003509575
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()>=0 && ci.getNumberOfPositionalLocks()<=4 && trial<=5 &&
     * solutions.size()>2)
     * trial distribution
     ...
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()>=0 && ci.getNumberOfPositionalLocks()<=4 && trial<=4 &&
     * solutions.size()>2)
     * trial distribution
     ...
     * <p>
     * Configuration:
     * if(ci.getNumberOfPositionalLocks()>=0 && ci.getNumberOfPositionalLocks()<=4 && trial<=3 &&
     * solutions.size()>2)
     * trial distribution
     * {1=1, 2=47, 3=806, 4=4758, 5=4708, 6=1869, 7=566, 8=188, 9=85, 10=43, 11=17, 12=10, 13=6,
     * 14=3}
     ...
     * Configuration
     * > Using char positional score for Advisor
     * > Using char freq score for ViableComputeEngine with scores of only UNIQUE PRIORITY
     * characters
     * trial distribution
